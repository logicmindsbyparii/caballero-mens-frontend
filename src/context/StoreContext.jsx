...
//   ];
//   // Safely falls back to localhost if the Vercel environment variable isn't detected locally
  ...
  const discountAmount = appliedCoupon 
    ? (appliedCoupon.type === 'percentage' 
        ? cart.reduce((sum, item) => {
            const reqCoupon = item.applicableCoupon?.trim().toUpperCase();
            if (reqCoupon && reqCoupon !== appliedCoupon.code.toUpperCase()) {
              return sum;
            }
            return sum + (item.price * item.quantity);
          }, 0) * (appliedCoupon.discount / 100)
        : (cart.some(item => !item.applicableCoupon?.trim() || item.applicableCoupon.trim().toUpperCase() === appliedCoupon.code.toUpperCase()) 
            ? appliedCoupon.discount 
            : 0))
    : 0;
